...
- The bridge supports one active `agy` command per server to avoid ambiguous shared CLI state. Do not introduce concurrent `run` or `continue` calls without defining the isolation guarantees.
